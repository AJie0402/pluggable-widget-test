...
function updateload() {
  const cell = this.data.getCell(this.data.selector.ri, this.data.selector.ci);
  const selectedText = cell && cell.text ? cell.text : "";
 
  const fileUrl = new URL(this.data.uri);
  const fileParams = new URLSearchParams(fileUrl.search);
  const fileGuid = fileParams.get('guid').toString();

  // 建立 excel , tempValue 設定為 selectedText
  if (typeof mx !== 'undefined' && mx.data && mx.data.create){
    // 創建實體
    mx.data.create({
      entity: "ExcelImporter.TemplateDocument", // 實體名稱
      callback: function(obj) {
        // window.alert("實體創建成功", obj);
        
        // 實體的屬性
        obj.set("tempValue", selectedText); // (key , string)
      ...
      //   window.alert("實體創建失敗:", error.message);
      // }
    });
  }
 ...
 //if (typeof mx !== 'undefined' && mx.data && mx.data.create){
    //// 創建實體
    // mx.data.create({
    //   entity: "LaboratoryManagement.formulaPdf", // 實體名稱
    //   callback: function(obj) {
    //     // window.alert("實體創建成功", obj);
        
    //     // 實體的屬性
    //     obj.set("Value", selectedText); // (key , string)
...
function sheetInitEvents() {
  ...
  contextMenu.itemClick = (type) => {
    ...
    } else if (type === 'updateload'){
      updateload.call(this);
    }else {
